                    ...
                    <!-- fieldset对表单进行分组-->
                        <fieldset>
                            <div class="form-group">
                                ...
                                <input id="name" class="form-control input-lg" placeholder="请输入用户名" name="name" type="text" autocomplete="off" autofocus>
                            </div>
...
<script>
	...
	function login(){
		 ...
		 //ajax提交登录方法
		 $.post("/mycomic/app/home/handler/login.handler.php",{name:name,password:password,role:role}, function(data){
            if( data.result == 1 ){
                alert('恭喜你,登录成功');
                window.location.href="/mycomic/"; 
            }else if(data.result == 2){
            	alert('恭喜你，管理员登录成功');
            	window.location.href="/mycomic/app/admin/index.php"; 
            }
            else{
                alert(data.message);
            }
